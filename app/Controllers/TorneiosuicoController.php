...
class TorneiosuicoController extends Controller
{
    ...
    public function gerenciar($id_torneio)
    {
        ...
        $this->view('torneio/gerenciarTorneioSuico', ['torneio' => $torneio, 'rodada' => $rodadaAtual, 'partidas' => $partidas, 'participantes' => $ranking, 'totalRodadas' => $totalRodadas, 'podeGerarProxima' => $podeGerarProxima, 'todasConcluidas' => $todasConcluidas]);
    }
    ...
    public function verPareamento($id_torneio, $numero_rodada)
    {
        ...
        if ($rodadaInfo)
        {
            ...
            foreach ($partidas as $p)
            {
                $pareamentos[] = ['jogador1' => $p['nome_j1'], 'jogador2' => $p['nome_j2']];
            }
        }
