...
class Produto
{
    ...
    {
        // O Autoload encontrará a classe Database no namespace Core
    ...
    public function buscar($id_produto, $id_loja)
    {
        $sql = "SELECT * FROM produtos
                ...
                AND id_loja = :id_loja";
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['id_produto' => $id_produto, 'id_loja' => $id_loja]);
    ...
    {
        $sql = "UPDATE produtos SET ativo = 1 WHERE id_produto = :id_produto AND id_loja = :id_loja";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute(['id_produto' => $id_produto, 'id_loja' => $id_loja]);
    }

    public function desativar($id_produto, $id_loja)
    {
        $sql = "UPDATE produtos SET ativo = 0 WHERE id_produto = :id_produto AND id_loja = :id_loja";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute(['id_produto' => $id_produto, 'id_loja' => $id_loja]);
    }
    ...
    public function atualizarOrdem($id_produto, $ordem)
    {
        ...
        $stmt = $this->db->prepare($sql);
        return $stmt->execute(['ordem' => $ordem, 'id_produto' => $id_produto]);
    }
    ...
    public function atualizarEstoque($id_produto, $quantidade)
    {
        ...
        $stmt = $this->db->prepare($sql);
        return $stmt->execute(['quantidade' => $quantidade, 'id_produto' => $id_produto]);
    }
